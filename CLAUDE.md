...
- **Content extraction**: `getContentValue` returns a string directly, prefers `#text`, and otherwise re-serializes child nodes back to XML with a module-level `XMLBuilder` (this is how inline XHTML `<content>` survives). `type="html"` content is additionally run through `decodeHtmlEntities`, a small hand-rolled decoder covering numeric, hex and the five predefined named entities.
- **Text constructs**: `getTypeContent` collapses all whitespace runs to single spaces and trims, so multi-line `<title>`/`<summary>` values are normalized. Assertions in tests assume this normalization.
