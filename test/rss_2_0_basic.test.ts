...
describe('Parsing RSS Channel information', () => {
    it('Test feed', () => {
        const rss = parseFeed(sample) as RSSChannel;
        ...
        expect(rss.pubDate).to.equal('Tue, 10 Jun 2003 04:00:00 GMT');
...
describe('Parsing RSS Items', () => {
    ...
    items.forEach((item, index) => {
        it(`Test item ${index + 1}`, () => {
            ...
            expect(rss.items[index].pubDate).to.equal(item.pubDate);
            expect(rss.items[index].guid).to.equal(item.guid);
        });
    });
