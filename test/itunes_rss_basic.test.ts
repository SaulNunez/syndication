...
describe('Parsing iTunes RSS Feed information', () => {
  it('Test feed', () => {
    ...
    expect(rss.copyright).to.equal('&#169; 2020 John Appleseed');
    expect(rss.itunes?.author).to.equal('The Sunset Explorers');
    expect(rss.description).to.equal(`Love to get outdoors and discover nature's treasures? Hiking Treks is the
      show for you. We review hikes and excursions, review outdoor gear and interview
      a variety of naturalists and adventurers. Look for new episodes each week.`);
    expect(rss.itunes?.type).to.equal('serial');
    expect(rss.itunes?.image).to.equal('https://applehosted.podcasts.apple.com/hiking_treks/artwork.png');
